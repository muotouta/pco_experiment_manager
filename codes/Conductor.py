...
class Conductor():
    ...
    def wait(self, num):
        ...
        unit = 1000  # 単位はミリ秒
        wait_time_sec = num / unit  # 秒に換算
        
        # step秒ごとにフラグを確認しながら待機
        step = 0.01
        elapsed = 0
        while elapsed < wait_time_sec:
            if not self.is_running:
                # 実行フラグが折れていたら例外を投げてprogramを中断させる
                raise InterruptedError("Stopped by user")
            
            sleep_time = min(step, wait_time_sec - elapsed)
    ...
    def program(self):
        ...
        speaker.set_value(0, 20000)
        speaker.set_value(1, 80)

        
        memo(f"pitch: 2000, volume: 80, exposure:2ms")
